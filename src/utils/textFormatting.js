export const BUILT_IN_FRENCH_ABBREVIATIONS = {
  c: "c'est",
  e: "est",
  g: "j'ai",
  j: "je",
...
const FRENCH_ACCENT_REPLACEMENTS = {
  aout: "août",
  apres: "après",
  bientot: "bientôt",
  cle: "clé",
...
export function normalizeAbbreviationKey(value) {
  return String(value || "")
    .trim()
    .toLowerCase()
    .normalize("NFD")
...
function normalizeAbbreviationExpansion(value) {
  return String(value || "").trim().replace(/\s+/g, " ");
}

function normalizeDictionaryRecord(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }

  return Object.entries(value).reduce((acc, [rawKey, rawExpansion]) => {
...
function readStoredAbbreviationDictionary() {
  ...
  try {
    ...
    const disabled = Array.isArray(parsed?.disabled)
      ? parsed.disabled
          .map((item) => normalizeAbbreviationKey(String(item || "")))
          .filter(Boolean)
      : [];
...
function saveStoredAbbreviationDictionary(data) {
...
export function readAbbreviationEntries() {
  const stored = readStoredAbbreviationDictionary();
  const disabled = new Set(stored.disabled);
  const keys = new Set([
...
export function upsertCustomAbbreviation(rawAbbreviation, rawExpansion) {
...
export function deleteAbbreviation(rawAbbreviation) {
  const abbreviation = normalizeAbbreviationKey(rawAbbreviation);
...
export function resetAbbreviation(rawAbbreviation) {
  const abbreviation = normalizeAbbreviationKey(rawAbbreviation);
  if (!abbreviation) return;
...
function looksLikePluralNoun(value) {
...
function resolveAmbiguousCAbbreviation(value) {
...
function readPreviousFrenchWords(value, endIndex, limit = 2) {
  const words =
    value
      .slice(0, endIndex)
...
function readNextFrenchWord(value, startIndex) {
  const match = value
    .slice(startIndex)
...
function readNextFrenchWords(value, startIndex, limit = 2) {
  const words =
    value
      .slice(startIndex)
      .match(new RegExp(FRENCH_WORD_PATTERN, "g")) || [];
...
function resolveAmbiguousEAbbreviation(value, wordStartIndex, wordEndIndex) {
...
function resolveAmbiguousKlAbbreviation(value, wordEndIndex, isPlural) {
...
function matchInitialCase(source, replacement) {
  return source[0] === source[0]?.toUpperCase()
    ? replacement.charAt(0).toUpperCase() + replacement.slice(1)
    : replacement;
}

function matchWordCase(source, replacement) {
...
function shouldRestoreAPreposition(value, offset) {
...
function restoreMissingFrenchAccents(value) {
...
function resolveQuelFormForNextWord(nextWord, fallback) {
...
function resolveQuelFormForVerb(verbWord, fallback) {
...
function resolveQuelFormForFollowingWords(nextWords, fallback) {
...
function resolveQuIlFormForVerb(verbWord, fallback) {
  const normalizedVerb = normalizeLookup(verbWord);
...
function resolveQuIlFormForFollowingWords(nextWords, fallback) {
  const nextWord = nextWords[0] || "";
  const normalizedNextWord = normalizeLookup(nextWord);
...
function resolveAmbiguousKlPhrases(value) {
...
function resolveAmbiguousIlPhrases(value) {
  const pattern = new RegExp(
...
function resolveQuelAgreement(value) {
  const pattern = new RegExp(
...
function expandFrenchAbbreviations(value, options = {}) {
...
export function formatTextSmart(value, options = {}) {
