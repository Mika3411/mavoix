...
import { ensureProfile } from "../../../shared/normalize";
...
async function resolveLocalPrivacyKey() {
  ...
  if (canUseIndexedDb()) {
    try {
      ...
      console.warn("IndexedDB indisponible pour la clé privée locale :", error);
    }
  }
...
async function decryptPrivatePayload(
  ...
  if (passwordProtected && !password) {
    throw new Error("Mot de passe requis pour déverrouiller les données médicales.");
  }
