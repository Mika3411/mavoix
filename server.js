...
function getCaregiverMessageHistory(channel) {
  ...
  pruneCaregiverMessageHistory(channel, history);
  return history;
}
...
function pruneCaregiverMessageHistory(channel, history) {
  if (!history) return;
...
<html lang="fr">
  <head>
    ...
    <style>
      :root { color-scheme: dark; font-family: Arial, system-ui, sans-serif; background: #0f172a; color: #f8fafc; }
      body { margin: 0; min-height: 100vh; display: grid; place-items: center; padding: 20px; box-sizing: border-box; }
      main { width: min(100%, 560px); display: grid; gap: 16px; }
      h1 { margin: 0; font-size: clamp(28px, 8vw, 44px); line-height: 1.05; }
      h2 { margin: 0; font-size: 24px; }
      label { color: #cbd5e1; font-size: 15px; font-weight: 800; }
      p { margin: 0; color: #cbd5e1; font-size: 18px; line-height: 1.5; }
      .card { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 22px; padding: 22px; box-shadow: 0 20px 50px rgba(0,0,0,0.28); }
      .status { border-radius: 16px; padding: 14px; background: rgba(59,130,246,0.16); border: 1px solid rgba(59,130,246,0.28); font-weight: 700; }
      .alert { background: rgba(220,38,38,0.25); border-color: rgba(248,113,113,0.45); animation: pulse 0.8s ease-in-out infinite alternate; }
      button, .file-label { min-height: 58px; border: 0; border-radius: 18px; padding: 14px 18px; color: white; font-size: 19px; font-weight: 800; cursor: pointer; box-sizing: border-box; }
      .primary { background: #2563eb; }
      .danger { background: #dc2626; }
      .secondary, .file-label { background: #334155; }
      .actions, .sound-settings, .messages, .message-form, .section-menu, .config-form { display: grid; grid-template-columns: 1fr; gap: 12px; }
      .section-select, .connection-link { min-height: 58px; width: 100%; border: 1px solid rgba(255,255,255,0.14); border-radius: 18px; padding: 0 16px; box-sizing: border-box; background: rgba(15,23,42,0.82); color: #f8fafc; font: inherit; font-size: 18px; font-weight: 800; }
      .section-panel[hidden] { display: none; }
      .connection-button { min-height: 132px; border-radius: 26px; font-size: clamp(26px, 8vw, 40px); line-height: 1.05; text-align: center; box-shadow: 0 22px 50px rgba(0,0,0,0.32); }
      .connection-button.connected { background: linear-gradient(135deg, #16a34a, #22c55e); color: #052e16; }
      .connection-button.disconnected { background: linear-gradient(135deg, #dc2626, #ef4444); color: #fff7ed; }
      .unread-list { display: grid; gap: 10px; }
      .unread-item { padding: 12px 14px; border-radius: 16px; background: rgba(34,197,94,0.16); border: 1px solid rgba(74,222,128,0.26); }
      .unread-meta { color: #bbf7d0; font-size: 12px; font-weight: 800; margin-bottom: 4px; }
      .unread-text { color: #f8fafc; font-size: 17px; line-height: 1.35; overflow-wrap: anywhere; }
      .unread-empty { color: #cbd5e1; font-size: 16px; line-height: 1.45; }
      .file-label { display: flex; align-items: center; justify-content: center; text-align: center; }
      .file-label input { display: none; }
      .sound-name { color: #cbd5e1; font-size: 16px; line-height: 1.45; }
      .message-list { min-height: 180px; max-height: 340px; overflow-y: auto; display: grid; align-content: start; gap: 10px; padding: 12px; border-radius: 18px; background: rgba(15,23,42,0.52); border: 1px solid rgba(255,255,255,0.08); }
      .message-empty, .message-status { color: #cbd5e1; font-size: 15px; line-height: 1.45; }
      .message-status.error { color: #fecaca; font-weight: 800; }
      .message-bubble { max-width: 86%; padding: 11px 13px; border-radius: 16px; display: grid; gap: 5px; line-height: 1.35; }
      .message-user { justify-self: start; background: rgba(34,197,94,0.18); border: 1px solid rgba(74,222,128,0.28); }
      .message-caregiver { justify-self: end; background: rgba(37,99,235,0.32); border: 1px solid rgba(96,165,250,0.38); }
      .message-meta { color: #cbd5e1; font-size: 12px; font-weight: 700; }
      .message-text { white-space: pre-wrap; overflow-wrap: anywhere; font-size: 17px; }
      .message-audio-button { min-height: 44px; border-radius: 14px; padding: 8px 12px; background: rgba(15,23,42,0.58); border: 1px solid rgba(255,255,255,0.16); color: #f8fafc; font-size: 16px; font-weight: 800; text-align: left; }
      .message-input { width: 100%; min-height: 110px; resize: vertical; border: 1px solid rgba(255,255,255,0.14); border-radius: 18px; padding: 14px; box-sizing: border-box; background: rgba(15,23,42,0.72); color: #f8fafc; font: inherit; font-size: 18px; }
  ...
  <body>
    ...
    <script>
      ...
      async function ensureAudioContext() {
        ...
        if (!AudioContextConstructor) throw new Error("Audio non disponible sur ce téléphone.");
      ...
      function openAlarmDb() {
        return new Promise((resolve, reject) => {
          if (!("indexedDB" in window)) return reject(new Error("Stockage audio indisponible sur ce téléphone."));
          const request = indexedDB.open(alarmDbName, 1);
          request.onupgradeneeded = () => request.result.createObjectStore(alarmStoreName);
      ...
      async function readStoredSound() {
        ...
        return new Promise((resolve, reject) => {
          const request = db.transaction(alarmStoreName, "readonly").objectStore(alarmStoreName).get(customSoundKey);
      ...
      async function saveStoredSound(file) {
        const db = await openAlarmDb();
        return new Promise((resolve, reject) => {
          const record = { name: file.name || "son d'alarme", type: file.type || "audio/mpeg", blob: file, savedAt: new Date().toISOString() };
          const request = db.transaction(alarmStoreName, "readwrite").objectStore(alarmStoreName).put(record, customSoundKey);
      ...
      async function clearStoredSound() {
        const db = await openAlarmDb();
        return new Promise((resolve, reject) => {
          const request = db.transaction(alarmStoreName, "readwrite").objectStore(alarmStoreName).delete(customSoundKey);
      ...
      async function loadStoredSound() {
        try { setCustomSound(await readStoredSound()); } catch { updateCustomSoundUi(); }
      }
      ...
      async function playDefaultAlarmTone() {
        ...
        if (navigator.vibrate) navigator.vibrate([280, 120, 280]);
      }
      ...
      async function startAlarmSound() {
        if (customAlarmUrl) {
          ...
          if (navigator.vibrate) {
            ...
            alarmInterval = window.setInterval(() => navigator.vibrate([280, 120, 280]), 1200);
          }
          return;
        }
        try {
          ...
          await playDefaultAlarmTone();
          alarmInterval = window.setInterval(() => void playDefaultAlarmTone(), 850);
        }
      }
      ...
      function connectMessages() {
        ...
        if (messageEvents) messageEvents.close();
      ...
      connectionButton.addEventListener("click", async () => {
        try {
          ...
          soundEnabled = true;
          if (!customAlarmUrl) await ensureAudioContext();
          await playAlarmOnce();
          connect();
          connectMessages();
      ...
      importSoundInput.addEventListener("change", async () => {
        ...
        try {
          ...
          if (soundEnabled) await playAlarmOnce();
        } catch (error) {
